    ...
    try:
        ...
        with connection() as conn:
            rows = conn.execute(
                ...
                "'service_categories',array_remove(ARRAY[ "
                "CASE WHEN hospitals_available THEN 'hospital' END, "
                "CASE WHEN grocery_stores_available THEN 'grocery_store' END, "
                "CASE WHEN libraries_available THEN 'library' END, "
                "CASE WHEN fire_stations_available THEN 'fire_station' END, "
                "CASE WHEN schools_available THEN 'school' END], NULL), "
